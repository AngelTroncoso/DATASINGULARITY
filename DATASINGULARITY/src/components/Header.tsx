...
import { Upload, Languages, Globe, User, LogOut, LogIn, Database } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const [datahubOnline, setDatahubOnline] = useState<boolean | null>(null);
  ...
  useEffect(() => {
    const checkDataHub = async () => {
      try {
        const response = await fetch('/api/datahub/status');
        const data = await response.json();
        setDatahubOnline(data.status === 'CONNECTED');
      } catch (err) {
        setDatahubOnline(false);
      }
    };

    checkDataHub();
    const interval = setInterval(checkDataHub, 30000); // Check every 30s
    ...
    <header className="bg-[#0A0A0C] border-b border-white/10 text-slate-300 px-6 py-2.5 sticky top-0 z-40 flex items-center justify-between select-none">
      <div className="w-full max-w-7xl mx-auto flex flex-col lg:flex-row items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-sm border text-[10px] font-mono tracking-widest uppercase transition-all ${
            datahubOnline === null
              ? 'bg-yellow-500/10 border-yellow-500/30 text-yellow-400'
              : datahubOnline
                ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400'
                : 'bg-rose-500/10 border-rose-500/30 text-rose-400'
          }`} title={datahubOnline === null ? 'Checking connection...' : datahubOnline ? 'Connected to DataHub GMS' : 'Start DataHub with: docker compose up -d'}>
            <Database className={`w-3 h-3 ${datahubOnline ? 'animate-pulse' : ''}`} />
            <span className="hidden sm:inline">
              {datahubOnline === null
                ? '...'
                : datahubOnline
                  ? t.header.datahubOnline
                  : t.header.datahubOffline}
            </span>
          </div>
